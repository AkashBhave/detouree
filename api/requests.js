...
async function req(origin, destination, block_temp) {
    ...
    if(block_temp.length==0){
        block_temp = [[[-74.24282736227363, 71.04737546410637],[-79.89422731090072, 104.97315587809182]]]}
    base =
    ...
    base = base + "&return=polyline,summary,actions,instructions&transportMode=pedestrian&avoid[areas]=bbox:";
    for (i in block_temp){
        block = block_temp[i]
        p1 = block[0]
        p2 = block[1]
   ...
   try{
    ...
    let point_array =   decode(data['routes'][0]['sections'][0]['polyline'])['polyline']
